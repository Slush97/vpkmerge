...
fn main() -> Result<()> {
    ...
    let corners = [[0.00195f32, 0.00195], [0.998, 0.998], [0.00195, 0.998], [0.998, 0.00195]];
    let is_hat = |uv: [f32; 2]| corners.iter().any(|c| (uv[0]-c[0]).abs() < 0.02 && (uv[1]-c[1]).abs() < 0.02);
    ...
    for (v, _) in &pts { for k in 0..3 { mn[k]=mn[k].min(v[k]); mx[k]=mx[k].max(v[k]); } }
    ...
    let mut hc=[0.0;3]; for v in &hp { for k in 0..3 { hc[k]+=v[k]; } }
    for k in 0..3 { hc[k]/= hp.len().max(1) as f32; }
    let tip = hp.iter().cloned().max_by(|a,b| {
    ...
    let al=(axis[0].powi(2)+axis[1].powi(2)+axis[2].powi(2)).sqrt().max(1e-6);
    let an=[axis[0]/al,axis[1]/al,axis[2]/al];
    eprintln!("native spans X{:.2} Y{:.2} Z{:.2}; up-axis={}", span[0],span[1],span[2], ["X","Y","Z"][up]);
    eprintln!("{} verts, {} hat. hat cone axis (centroid->tip) native=({:+.2},{:+.2},{:+.2})", pts.len(), hatn, an[0],an[1],an[2]);
    eprintln!("  -> |up-component|={:.2} (1.0=straight up, ~0=lying sideways)", an[up].abs());
    ...
    let mut vmn=[f32::INFINITY;3]; let mut vmx=[f32::NEG_INFINITY;3];
    for pr in &view { for k in 0..3 { vmn[k]=vmn[k].min(pr.0[k]); vmx[k]=vmx[k].max(pr.0[k]); } }
    let map=|val:f32,lo:f32,hi:f32,a:f32,b:f32| a+(val-lo)/(hi-lo+1e-6)*(b-a);
    let mut plot=|hx:f32,vz:f32,haxis:usize,panel:u32,col:[u8;3],img:&mut [u8]|{
        ...
        if ix>=0&&ix<w as i32&&iy>=0&&iy<hh as i32 {
            let o=((iy as u32*w+ix as u32)*3) as usize; img[o]=col[0];img[o+1]=col[1];img[o+2]=col[2];
        }
    };
    for pr in &view { let (p,hat)=**pr; if !hat {
    ...
    eprintln!("wrote {out}  LEFT=({},{}) RIGHT=({},{}). red=hat", ["X","Y","Z"][h1],["X","Y","Z"][up],["X","Y","Z"][h2],["X","Y","Z"][up]);
    Ok(())
}
