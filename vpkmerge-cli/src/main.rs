...
struct MetadataCmd {
    ...
    /// Addon title (`addontitle`). Required (with `--author`) whenever any
    /// typed addoninfo field is given, so a generated `addoninfo.txt` can
    /// never carry empty identity fields. Omit every typed field when
    /// bringing your own `addoninfo.txt` via `--extra-file`.
    #[arg(long, value_name = "TITLE", requires = "author")]
    title: Option<String>,

    /// Addon author (`addonauthor`). Required together with `--title`.
    #[arg(long, value_name = "AUTHOR", requires = "title")]
    author: Option<String>,

    /// Addon version (`addonversion`). Defaults to "1.0" if not given.
    /// Requires `--title`/`--author`.
    #[arg(long, value_name = "VERSION", requires = "title")]
    version: Option<String>,

    /// Short description (`addonDescription`). Requires `--title`/`--author`.
    #[arg(long, value_name = "TEXT", requires = "title")]
    description: Option<String>,

    /// Numeric `GameBanana` submission id this VPK was downloaded from, kept
    /// as a string so it round-trips verbatim. Requires `--title`/`--author`.
    #[arg(long = "gamebanana-id", value_name = "ID", requires = "title")]
    gamebanana_id: Option<String>,

    /// Source page URL, e.g. the `GameBanana` listing this VPK came from.
    /// Requires `--title`/`--author`.
    #[arg(long = "source-url", value_name = "URL", requires = "title")]
    source_url: Option<String>,

    /// Build timestamp, e.g. an ISO 8601 string from the caller's own clock.
    /// Not generated by this tool. Requires `--title`/`--author`.
    #[arg(long = "build-date", value_name = "DATE", requires = "title")]
...
fn run_metadata(args: &MetadataCmd) -> Result<()> {
    ...
    // pass --title/--author for a generated addoninfo.txt. A typed block is
    // never generated with empty identity fields: clap's requires-chain forces
    // --title/--author whenever any typed field is present, and the bail below
    // is defense-in-depth for non-clap callers.
    let metadata = match (&args.title, &args.author) {
        (Some(title), Some(author)) => Some(AddonMetadata {
            title: title.clone(),
            author: author.clone(),
        ...
        (None, None)
            if args.version.is_none()
                ...
                && args.gamebanana_id.is_none()
                && args.source_url.is_none()
                && args.build_date.is_none() =>
        {
            None
        }
        _ => anyhow::bail!("--title and --author are required when any addoninfo field is given"),
    };
...
mod tests {
    ...
    #[test]
    fn metadata_lone_build_date_is_rejected() {
        // The exact review reproduction: a lone typed flag must not mint an
    ...
    #[test]
    fn metadata_title_without_author_is_rejected() {
        let err = Cli::try_parse_from([
            "vpkmerge", "metadata", "--vpk", "in.vpk", "--title", "T", "--output", "out.vpk",
        ])
        .map(|_| ())
        .unwrap_err();
    ...
    #[test]
    fn metadata_author_without_title_is_rejected() {
        let err = Cli::try_parse_from([
            "vpkmerge", "metadata", "--vpk", "in.vpk", "--author", "A", "--output", "out.vpk",
        ])
        .map(|_| ())
        .unwrap_err();
        assert_eq!(err.kind(), clap::error::ErrorKind::MissingRequiredArgument);
    ...
    #[test]
    fn metadata_extra_file_only_still_parses() {
        // Extras-only and drop-only invocations carry no typed block and must
        // stay legal (Grimoire's own-addoninfo path).
        let cli = Cli::try_parse_from([
            ...
            "in.vpk",
            "--extra-file",
            "grimoire_meta.json=meta.json",
            "--drop-entry",
            "./stale.json",
            "--output",
            "out.vpk",
        ]);
        assert!(cli.is_ok(), "got: {:?}", cli.err().map(|e| e.to_string()));
    }

    #[test]
    fn metadata_full_typed_block_parses() {
        let cli = Cli::try_parse_from([
            "vpkmerge",
            "metadata",
            "--vpk",
