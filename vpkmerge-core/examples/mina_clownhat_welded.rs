...
use vpkmerge_core::soul_import_clone::{base_color_factor, glb_json, material_index_by_name, read_glb_primitives};
...
fn main() -> Result<()> {
    ...
    let crown_z = shared.positions.iter().map(|p| p[2]).fold(f32::NEG_INFINITY, f32::max);
    eprintln!("head mesh top (hair) z = {crown_z:.1}; head bone z = {:.1}", anchor[2]);
    let (face_idx, face_prim) = head
        .primitives
        .iter()
        .enumerate()
        .find(|(_, p)| p.material.rsplit('/').next().is_some_and(|f| f.starts_with("mina_head") && !f.contains("_ui")))
    ...
    for (pi, p) in prims.iter().enumerate() {
        if let Some(g) = groups.iter_mut().find(|g| g.0 == p.material_name) {
            ...
            let col = p.material_name.as_ref().and_then(|m| mat_index.get(m))
    ...
    let mut hat = VertexBuffer { texcoords: vec![Vec::new()], ..VertexBuffer::default() };
    ...
    for (gi, g) in groups.iter().enumerate() {
        for &pi in &g.1 {
            ...
            for v in &vb.positions { hat.positions.push([v[0], v[2], -v[1]]); }
            for v in &vb.normals { hat.normals.push([v[0], v[2], -v[1]]); }
            for _ in 0..vb.positions.len() { vgroup.push(gi); }
    ...
    for p in &hat.positions { for k in 0..3 { mn[k] = mn[k].min(p[k]); mx[k] = mx[k].max(p[k]); } }
    ...
    for nv in &mut hat.normals { *nv = mat3_mul(&rot, nv); }
    ...
    for tri in face_prim.indices.chunks_exact(3) {
        let uv: Vec<[f32; 2]> = tri.iter().map(|&i| shared.texcoords[0][i as usize]).collect();
    ...
    let ImageData::Rgba8(mut px) = img.data else { return Err(anyhow!("albedo is HDR")) };
    let cell_w = w / GRID;
    let cell_h = h / GRID;
    for (gi, &(cx, cy)) in free.iter().enumerate() {
        let col = groups[gi].2;
        let rgb = [to_srgb_u8(col[0]), to_srgb_u8(col[1]), to_srgb_u8(col[2]), 255];
    ...
    let new_albedo = morphic::replace_mip_chain(
        &albedo_c,
        &Image { width: w as u32, height: h as u32, data: ImageData::Rgba8(px) },
    ).map_err(|e| anyhow!("re-encode albedo: {e}"))?;

    // --- assign hat UVs to each group's swatch-cell center ---
    let swatch_uv: Vec<[f32; 2]> = free.iter().map(|&(cx, cy)| {
        [(cx as f32 + 0.5) / GRID as f32, (cy as f32 + 0.5) / GRID as f32]
    }).collect();
    ...
    let sel = PrimitiveSelection { mesh_index: head.mesh_index, primitive_index: face_idx };
    ...
    vpkmerge_core::pack(&[(ENTRY, edited.as_slice()), (ALBEDO_C, new_albedo.as_slice())], &out)?;
    eprintln!("wrote {out}: model + overridden face albedo (hat welded into mina_head, {vc} hat verts)");
    Ok(())
}
...
fn to_srgb_u8(lin: f64) -> u8 {
    let s = if lin <= 0.003_130_8 { 12.92 * lin } else { 1.055 * lin.powf(1.0 / 2.4) - 0.055 };
...
fn rasterize_tri(used: &mut [bool], uv: &[[f32; 2]]) {
    ...
    for q in &p { for k in 0..2 { mn[k] = mn[k].min(q[k]); mx[k] = mx[k].max(q[k]); } }
    ...
    for gy in y0..y1 {
        for gx in x0..x1 {
            let c = [(gx as f32 + 0.5) / GRID as f32, (gy as f32 + 0.5) / GRID as f32];
...
fn point_in_tri(pt: [f32; 2], t: &[[f32; 2]]) -> bool {
    let d = |a: [f32; 2], b: [f32; 2], c: [f32; 2]| (a[0] - c[0]) * (b[1] - c[1]) - (b[0] - c[0]) * (a[1] - c[1]);
    let d1 = d(pt, t[0], t[1]);
    let d2 = d(pt, t[1], t[2]);
    let d3 = d(pt, t[2], t[0]);
...
fn dilate(used: &mut [bool], r: usize) {
    for _ in 0..r {
        ...
        for gy in 0..GRID {
            for gx in 0..GRID {
                if snap[gy * GRID + gx] { continue; }
                let near = [(0i32, 1i32), (0, -1), (1, 0), (-1, 0)].iter().any(|&(dx, dy)| {
                    let nx = gx as i32 + dx;
                    let ny = gy as i32 + dy;
                    nx >= 0 && nx < GRID as i32 && ny >= 0 && ny < GRID as i32 && snap[ny as usize * GRID + nx as usize]
                });
                if near { used[gy * GRID + gx] = true; }
            }
        }
    }
}
...
fn find_free_cells(used: &[bool], n: usize) -> Option<Vec<(usize, usize)>> {
    ...
    for gy in 0..GRID {
        for gx in 0..GRID {
            if !used[gy * GRID + gx] { frees.push((gx, gy)); }
        }
    }
    if frees.len() < n { return None; }
    // Greedy farthest-point spread for separation.
    let mut chosen = vec![frees[0]];
    while chosen.len() < n {
        let next = frees.iter().copied().max_by_key(|&(x, y)| {
            chosen.iter().map(|&(cx, cy)| (x as i64 - cx as i64).pow(2) + (y as i64 - cy as i64).pow(2)).min().unwrap_or(0)
        })?;
        chosen.push(next);
    }
    Some(chosen)
...
fn compact(shared: &VertexBuffer, indices: &[u32]) -> (VertexBuffer, Vec<u32>) {
    ...
    for &gi in indices {
        let id = *map.entry(gi).or_insert_with(|| { let i = order.len() as u32; order.push(gi); i });
        reb.push(id);
    }
    let nn = shared.element_count;
    let p3 = |s: &[[f32; 3]]| if s.len() == nn { order.iter().map(|&g| s[g as usize]).collect() } else { Vec::new() };
    let p4 = |s: &[[f32; 4]]| if s.len() == nn { order.iter().map(|&g| s[g as usize]).collect() } else { Vec::new() };
    let vb = VertexBuffer {
        ...
        texcoords: shared.texcoords.iter().map(|ch| if ch.len() == nn { order.iter().map(|&g| ch[g as usize]).collect() } else { Vec::new() }).collect(),
        colors: shared.colors.iter().map(|ch| if ch.len() == nn { order.iter().map(|&g| ch[g as usize]).collect() } else { Vec::new() }).collect(),
        joints: if shared.joints.len() == nn { order.iter().map(|&g| shared.joints[g as usize]).collect() } else { Vec::new() },
        weights: if shared.weights.len() == nn { order.iter().map(|&g| shared.weights[g as usize]).collect() } else { Vec::new() },
...
fn append(vb: &mut VertexBuffer, hat: &VertexBuffer, hat_idx: &[u32], base: u32, idx: &mut Vec<u32>) {
    ...
    for i in 0..hat.element_count {
        ...
        if has_tan { vb.tangents.push([1.0, 0.0, 0.0, 1.0]); }
        for ch in 0..n_uv {
            vb.texcoords[ch].push(hat.texcoords.first().map_or([0.5, 0.5], |t| t[i]));
        }
        for ch in 0..n_col { vb.colors[ch].push([1.0; 4]); }
