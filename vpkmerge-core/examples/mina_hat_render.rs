...
fn main() -> Result<()> {
    ...
    let out = a.next().unwrap_or_else(|| ".scratch/mina_hat_render.png".into());
    ...
    for p in &hat { for k in 0..3 { mn[k]=mn[k].min(p[k]); mx[k]=mx[k].max(p[k]); } }
    ...
    for p in &hat { cx+=p[0]; cy+=p[1]; lo=lo.min(p[2]); }
    cx/=hat.len() as f32; cy/=hat.len() as f32;
    let (sx, sy, sz) = (anchor[0]-cx, anchor[1]-cy, crown_z+raise-lo);
    for p in &mut hat { p[0]+=sx; p[1]+=sy; p[2]+=sz; }

    // head points to draw (face + hair prims), in model space.
    let mut head_pts: Vec<[f32;3]> = Vec::new();
    for p in &head.primitives {
        for &i in &p.indices { head_pts.push(shared.positions[i as usize]); }
    }
    ...
    let pad = 30.0f32;
    // common z range
    let zlo = 70.0; let zhi = 110.0;
    let xlo = -25.0; let xhi = 25.0; // covers head (-9..5) + hat
    ...
    for gx in 0..w { for &(zz,cc) in &[(crown_z,[60u8,60,90]),(anchor[2],[40,70,40])] {
        let py = to_px(zz, zhi, zlo, pad, h as f32-pad) as i32;
        if py>=0 && py<h as i32 { let o=((py as u32*w+gx)*3) as usize; img[o]=cc[0];img[o+1]=cc[1];img[o+2]=cc[2]; }
    }}
    ...
    for p in &hat { for k in 0..3 { hn[k]=hn[k].min(p[k]); hxb[k]=hxb[k].max(p[k]); } }
    eprintln!("anchor head bone z={:.1}, crown(hair top) z={:.1}", anchor[2], crown_z);
    eprintln!("HAT final bbox: x[{:.1},{:.1}] y[{:.1},{:.1}] z[{:.1},{:.1}]  (w/d/h {:.1}/{:.1}/{:.1})",
        hn[0],hxb[0],hn[1],hxb[1],hn[2],hxb[2], hxb[0]-hn[0],hxb[1]-hn[1],hxb[2]-hn[2]);
